...
public class Trigger : IValidatableObject
{
      ...
      if (string.IsNullOrEmpty(ApiKeyFieldName))
      {
        switch (ApiKeyInsertLocationId)
        {
          ...
          case (int)ApiKeyInsertLocation.Header:
            yield return new ValidationResult("The header name is missing.", [nameof(ApiKeyFieldName)]);
            break;
        }
      }
      if (ApiKeyId == null)
      {
        yield return new ValidationResult("The API Key has not been selected.", [nameof(ApiKeyId)]);
      }
    }
  }
}
