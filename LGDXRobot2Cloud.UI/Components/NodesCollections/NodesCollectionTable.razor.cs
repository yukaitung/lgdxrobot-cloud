...
namespace LGDXRobot2Cloud.UI.Components.NodesCollections
{
  public partial class NodesCollectionTable : AbstractTable
  {
    ...
    private List<NodesCollectionBlazor>? NodesCollectionsList { get; set; }
    ...
    protected override async Task HandlePageSizeChange(int number)
    {
      ...
      if (PageSize > 100)
        ...
        PageSize = 1;
      var data = await NodesCollectionService.GetNodesCollectionsAsync(DataSearch, 1, PageSize);
      NodesCollectionsList = data.Item1?.ToList();
    ...
    protected override async Task HandlePageChange(int pageNum)
    {
      ...
      if (pageNum > PaginationMetadata?.PageCount || pageNum < 1)
        return;
      var data = await NodesCollectionService.GetNodesCollectionsAsync(DataSearch, pageNum, PageSize);
      NodesCollectionsList = data.Item1?.ToList();
      PaginationMetadata = data.Item2;
    }
    ...
    {
      if (deleteOpt && CurrentPage > 1 && NodesCollectionsList?.Count == 1)
        CurrentPage--;
      var data = await NodesCollectionService.GetNodesCollectionsAsync(DataSearch, CurrentPage, PageSize);
      NodesCollectionsList = data.Item1?.ToList();
      PaginationMetadata = data.Item2;
    }
